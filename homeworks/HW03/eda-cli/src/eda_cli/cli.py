...
def report(
    ...
    top_cats = top_categories(df, top_k = top_k_categories) #здесь взаимосвязано с core. передается значение пользователя либо дефолтное
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write("## Слишком много нулей. \n\n")
        if quality_flags.get('has_many_zero_values'):
            names = ", ".join(col["column"] for col in quality_flags["zero_columns"])
            f.write(f"Слишком много нулей в колонках: {names}")

        if quality_flags.get("has_constant_columns"):
            names = ", ".join(quality_flags["constant_columns"])
            f.write(f"- Постоянные колонки: **{names}**\n")
