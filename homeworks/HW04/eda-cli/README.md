...
---
### 4. `POST /quality-flags-from-csv` – оценка качества по CSV-файлу
...
- вызывает функции из `eda_cli.core`:
 ...
 - возвращает **полный набор флагов качества**, включая новые.
...
Ответ будет содержать json вида:
```json
{
"latency_ms": 5.5,
  "flags": {
...
- 'too_few_rows' - флаг о недостаточном количестве строк датасета.
- 'too_many_columns' - флаг о слишком большом количестве колонок в датасете.
- 'has_constant_values' - флаг о том, что в колонке находятся одни и те же значения.
- 'has_many_zero_values' - флаг о том, что есть колонки со слишком большим процентом нулей.
- `latency_ms` - время обработки запроса.
