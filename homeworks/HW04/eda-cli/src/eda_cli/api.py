...
class QualityResponseFlags(BaseModel):
    latency_ms: float = Field(
        ...,
        ge=0.0,
        description="Время обработки запроса на сервере, миллисекунды",
...
# ---------- /quality-flags-from-csv: вывод набора флагов качества ----------

@app.post("/quality-flags-from-csv", response_model=QualityResponseFlags, tags=["quality"], summary="Выставление флагов качества",)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityResponseFlags:
