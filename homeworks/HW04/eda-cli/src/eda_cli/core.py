...
def summarize_dataset(
    ...
    for name in df.columns:
        ...
        has_constant_values = (unique <= 1) and (non_null > 0)
        ...
        if is_numeric and non_null > 0:
            ...
            zero_share = zero_count / len(s) if len(s) > 0 else 0.0
            big_zero_share = zero_share >= 0.5
...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    constant_columns = [col.name for col in summary.columns if col.has_constant_values]
    flags["has_constant_values"] = len(constant_columns) > 0
    flags["constant_columns"] = constant_columns

    zero_columns = [
        {"column": col.name, "zero_share": col.zero_share}
        for col in summary.columns
        if col.is_numeric and col.zero_share >= 0.5
    ]
        ...
        rows.append(
            {
                ...
                "mean": col.mean,
                "std": col.std,
                "has_many_zero_values": col.big_zero_share,
                'has_constant_values': col.has_constant_values
            }
        )
